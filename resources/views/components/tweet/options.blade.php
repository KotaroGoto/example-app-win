        ...
        <div class="bg-white rounded shadow-md absolute right-0 w-24 z-20 pt-1 pb-1">
            <div>
                <a href="{{ route('tweet.update.index', ['tweetId' => $tweetId]) }}"
                    class="flex items-center gap-1 pt-1 pb-1 pl-3 pr-3 hover:bg-gray-100">
            ...
            <div>
                <form action="{{ route('tweet.delete', ['tweetId' => $tweetId]) }}" method="post"
                    ...
                    @method('DELETE')
                    @csrf
                    <button type="submit" class="flex items-center gap-1 w-full pt-1 pb-1 pl-3 pr-3 hover:bg-gray-100">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
